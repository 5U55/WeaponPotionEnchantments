...
public class WeaknessEnchant extends Enchantment {

	public WeaknessEnchant() {
	...
	public void onTargetDamaged(LivingEntity user, Entity target, int level) {
		if(target instanceof LivingEntity) {
			((LivingEntity) target).addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 200*level, level-1));
		}
	...
	public boolean canAccept(Enchantment other) {
		return super.canAccept(other)  && other != PotionedWeapons.HEALING_ENCHANT
				...
				&& other != PotionedWeapons.HARMING_ENCHANT;
	}
}
